...
# Check if we're in dry-run mode to skip env var validation
import sys
is_dry_run_args = any(arg in sys.argv for arg in ['--dry-run'])

if not is_dry_run_args and (not GIST_ID or not TOKEN):
...
def get_quotes_en(num_quotes: int = 2) -> List[str]:
    ...
    # Check if we're in dry-run mode - if so, skip API calls and use fallback
    is_dry_run = getattr(args, 'dry_run', False) if 'args' in globals() else False
    
    if is_dry_run:
        print("🔍  DRY RUN - Using fallback quotes, skipping API calls")
        fallback_quotes = [
